...
export default function CategoryPostsPage() {
  ...
  const categoryKey = Array.isArray(categoryId) ? categoryId[0] : categoryId;
  const firestore = useFirestore();

  const categoryRef = useMemoFirebase(() => {
    if (!firestore || !categoryKey) return null;
    return doc(firestore, 'categories', categoryKey);
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8">
      <section className="text-center py-16 sm:py-24">
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-headline font-bold tracking-tight text-primary">
          Category: {category?.name || categoryKey || 'All'}
        </h1>
        <p className="mt-4 max-w-2xl mx-auto text-lg text-muted-foreground">
          Articles in {category?.name || 'this category'}.
        </p>
      </section>
